...
int rdp_send(struct udp_pcb *pcb , struct pbuf *p){
    ...
    //add a packet to the window awaiting an ack
    int e0 = add_pkt_window(pcb, p);
    if (e0 != ERR_OK){
        printf("Failed to add packet to window, could not send packet.\n");
        return 1;        
    }
...
void rdp_recv_callback(void *arg, struct udp_pcb *pcb, struct pbuf *p, uchar *addr, uint16_t port_raw) {
    uint16_t port = port_raw;

    //bit mask to check if ack
    int k = 15;
    int mask = 1 << k;
    int masked_n = port & mask;
    int flagbit = masked_n >> k;
    ...
    if(flagbit == 1){
        ...
        //delete from the window list waiting
        int e0 = del_pkt_window(pcb, p);
            if(e0 != ERR_OK){
                //printf("Could not delete packet matching ACK from window.\n");
            }
            else{
                //printf("An outstanding packet >>%s was resolved.\n", p->payload);
            }        
    } 
    //is not an ACK
    else{ 
...
void send_ack(struct udp_pcb *pcb, struct pbuf *p){
    ...
    //Temp most of local_port so the src port gets changed
    int local_port = pcb->local_port;
    pcb->local_port = local_port + 32768;

    // create pbuf and call udp_send()
    struct pbuf *Xp = pbuf_alloc(PBUF_TRANSPORT, strlen(p->payload), PBUF_RAM);
    Xp->payload = p->payload;
    err_t e1 = udp_send(pcb, Xp);  
                if (e1 != ERR_OK)
                    printf("udp send error: %d\n", e1);

    //port back afterwards
    pcb->local_port = local_port;

    pthread_mutex_unlock(&win_x);

    printf("An ACK was sent.\n");
}
...
void gncCmd() {
    ...
    // -r for RDP
        ...
        if (!strcmp(next_tok, "-l")) {
            ...
            //create a thread that will manage our timers and respond to timeout
            pthread_t thread_id;
            thread_state = 1;
            pthread_create(&thread_id, NULL, thread_timer_actions, NULL);
            pthread_mutex_init(&win_x, NULL);
